...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PCR, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (paycore.conf).");
            QMessageBox::warning(this, "Paycore Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; background-color: #5b55a3; color:#fff;}\n a, span { font-family: monospace; }\n span.addr {color:#fff; font-weight: bold;}\n table tr td {padding: 3px; border: none; background-color: #5b55a3;}\n td.d0 {font-weight: bold; color:#fff;}\n h2, h3 { white-space:nowrap; color:#333;}\n a { text-decoration:none; }\n a.nav {color:green;}\n";
