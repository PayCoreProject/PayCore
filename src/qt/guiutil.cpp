...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PCR address (e.g. %1)").arg("PndX228fqyNdTRh8PmhkLeHga9siLY8tbD"));
#endif
...
{
    // return if URI is not valid or is no PCR: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::PCR, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert paycore:// to paycore:
    //
    //    Cannot handle this later, because paycore:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PCR, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open paycore.conf with the associated application */
                                                                  ...
                                                                  value(amount)
{
    ...
    if (amount > 0)
    {
        res = BitcoinUnits::floorWithUnit(BitcoinUnits::PCR, amount);
    }
    else
    {
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "PCR.lnk";
}
...
{
    return GetAutostartDir() / "paycore.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a paycore.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the paycore app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add paycore app to startup item list
